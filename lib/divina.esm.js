import { Container as Container$2, Graphics, autoDetectRenderer, utils, TextStyle, Text, TextMetrics, Ticker, Texture as Texture$2, Sprite, AnimatedSprite, Rectangle, Loader as Loader$2, LoaderResource, BaseTexture as BaseTexture$1 } from 'pixi.js';
...
class TextBlock$1 extends Container$1 {
	...
	constructor(name, parent, text, textStyle, rect, lineWidth, boundingRect) {
		const finalStyle = new TextStyle(textStyle);
		const pixiTextContainer = new Text("", finalStyle);
		...
		resultsArray.forEach((substring) => {
			...
			if (string === " " && isNewLine === false) {
				...
				if (isNewLine === true
					...
					// Get rid of spaces
					actualString = string;
					textMetrics = TextMetrics.measureText(string, actualTextStyle);
				}
...
class Player {
	...
	constructor(rootElement, backgroundColor = null) {
		...
		const shouldReturnDefaultValue = true;
		const value = null;
		this._continuous = returnValidValue("continuous", value, shouldReturnDefaultValue);
		this._spread = returnValidValue("spread", value, shouldReturnDefaultValue);
	...
	_sizeViewport(width, height) {
		...
		if (this._pageNavigator
			...
			this._readingModesArray = Object.keys(actualReadingModes);
			data.readingModesArray = this._readingModesArray;
	...
	setReadingMode(readingMode) {
		if (!readingMode || this._readingModesArray.includes(readingMode) === false) {
			return
		}
	...
	_buildStoryFromPageNavigatorsData(pageNavigatorsData, locator) {
		...
		const data = {
			...
			continuous,
			readingModesArray: this._readingModesArray,
			languagesArray,
		};
		...
		this._setPageNavigator(readingMode, locator);
	}
	...
	_setPageNavigator(pageNavType, locator = null) {
		...
		if (this._resourceManager.haveFirstResourcesLoaded === false) {
			...
			const forcedNb = (initialNbOfResourcesToLoad !== undefined
				&& isANumber(initialNbOfResourcesToLoad) === true
	...
	_getTargetFromHref(readingMode, targetHref, canUseShortenedHref = false) {
		if (!targetHref || isAString(targetHref) === false) {
