import { Container as Container$2, Graphics, Application, utils, Text, Ticker, Texture as Texture$2, Sprite, AnimatedSprite, Rectangle, Loader as Loader$2, LoaderResource, BaseTexture as BaseTexture$1 } from 'pixi.js';
import { Manager, Tap, Pinch, Swipe, DIRECTION_ALL, Pan } from 'hammerjs';
...
class Container {
	...
	constructor(type = null, name = null, parent = null, pixiContainer = null) {
		this._type = type;
		this._pixiContainer = pixiContainer || new Container$2();
		this._setName(name);
		if (parent) {
	...
	addMask() {
		this._maskingPixiContainer = new Container$2();
		this._setMaskingPixiContainerName();
	...
	addChildAtIndex(container, index) {
		...
		const tmpPixiContainer = new Container$2();
		i -= 1;
		while (i >= 0 && zIndex <= i) {
...
const POSSIBLE_TAG_NAMES = ["language"]; // List of allowed tags ("type" can also have variations) // IF YOU ADD ANY, WILL NEED TO BE ADDED TO SLICERESOURCE (props and getters) TOO, for SLICERESOURCE!!!
const DEFAULT_BACKGROUND_COLOR = "#000000"; // Black, to ensure the loading message is visible
...
const TEXT_FILL_COLOR = "#FFFFFF"; // White, to ensure the message is visible
const WORD_WRAP_WIDTH = 275; // Maximum line width
...
// Story
const DEFAULT_DURATION = 750; // In milliseconds (used for transitions and snap point jumps)
...
class Renderer {
	...
	constructor(rootElement, backgroundColor = null) {
		...
		if (options.transparent !== true) {
			const defaultColor = convertColorStringToNumber(DEFAULT_BACKGROUND_COLOR);
...
class TextElement extends Container {
	...
	constructor(name, parent) {
		...
		const pixiTextContainer = new Text("", {
			fontFamily: TEXT_FONT_FAMILY,
			fontSize: TEXT_FONT_SIZE,
			fill: convertColorStringToNumber(TEXT_FILL_COLOR),
			wordWrap: true,
...
class LoadingAnimation {
	...
	_updateGraphics() {
		...
		this._loadingAnimationGraphics = new Graphics();
		...
		const lineTextureStyle = { width: LINE_WIDTH, texture: Texture$2.WHITE };
...
class TextureElement extends Container {
	...
	_setTexture(texture) {
		...
		if (!texture) {
			this._sprite.texture = Texture$2.WHITE;
			const dummyColor = convertColorStringToNumber(DEFAULT_DUMMY_COLOR);
	...
	_setVideoTexture(videoTexture) {
		...
		this._playableSprite = Sprite.from(videoTexture.pixiTexture);
	...
	setTexturesArray(texturesArray) {
		...
		this._playableSprite = new AnimatedSprite(pixiTexturesArray);
		this._addPlayableSprite();
	}
...
class Texture {
	...
	static createVideoTexture(videoPath) {
		const pixiTexture = Texture$2.from(videoPath);
...
class Loader {
	...
	add(resourceId, sourcePath) {
		...
		this._loader.add(name, sourcePath, {
			crossOrigin: "anonymous", loadType: LoaderResource.LOAD_TYPE.IMAGE,
		});
	}
		...
		this._loader.onComplete.add((_, resources) => {
			...
			Object.values(resources).forEach((resource) => {
				...
				if (this._resourceIdsToLoad[resourceId] === true && texture && texture.baseTexture) {
					...
					Texture$2.removeFromCache(texture.pixiTexture);
					BaseTexture$1.removeFromCache(baseTexture.pixiBaseTexture);
				}
			});
...
class TextManager extends Container$1 {

	constructor(mainContainer) {
		super("textManager", "textManager", mainContainer);

		this._textElement = new TextElement$1("textElement", this);
	}
	...
	showMessage(message) { // Where message should be = { type, data }
		...
		switch (type) {
		case "loading":
			text = `${DEFAULT_LOADING_MESSAGE}... ${data}%`;
			break
		case "error":
			text = `ERROR!\n${data}`;
			break
...
class InteractionManager {
	...
	constructor(player, rootElement) {
		...
		this._mc = new Manager(rootElement);

		// Implement single and double tap detection
		const singleTap = new Tap({ event: "singletap" });
...
class TextureResource extends CoreResource {
	...
	addOrUpdateFragment(fragment = "full", sliceId) {
		...
		if (this._baseTexture && this._textures.full && this._textures.full.texture
			...
			const croppedTexture = Texture$1.cropToFragment(fullTexture, fragment);
	...
	setActualTexture(textureData) { // textureData = { name, texture }
		...
		if (this._loadStatus === -1) {
			...
			if (this._fallbackFragment) {
				const croppedTexture = Texture$1.cropToFragment(full, this._fallbackFragment);
				Object.keys(this._textures).forEach((fragment) => {
					this._textures[fragment].texture = croppedTexture;
				});
...
class VideoTextureResource extends TextureResource {
	...
	_doOnDurationChange(resolve, doOnVideoLoadSuccess) {
		...
		if (this._loadStatus !== 0 // Loading may indeed have been cancelled
			...
			const { baseTexture, texture } = Texture$1.createVideoTexture(this._video);
			const textureData = {
				resourceId: this._id,
				texture: {
					base: baseTexture,
...
class Slice extends TextureElement$1 {
...
class ResourceBuilder {
	...
	static createResourceInfoArray(object, player) {
		...
		const resourceType = getResourceType(path, type);
	...
	static _handleAlternateArray(object, alternateArray, oldAltParts, parentResource,
		...
		alternateArray.forEach((alternateObject) => {
			if (alternateObject && alternateObject.href
				&& isAString(alternateObject.href) === true) {
...
class Transition {
	...
	static createTransition(transition, player) {
		...
		const actualDuration = returnValidValue("positive", duration, shouldReturnDefaultValue);
		if (actualDuration) {
		...
		if (actualType === "animation") {
			if (file && isAnObject(file) === true) {
				...
				sliceProperties.duration = returnValidValue("positive", duration,
					shouldReturnDefaultValue);
...
class Camera {
	...
	setBoundsAndUpdateOnResize() {
		...
		const callback = () => {
			...
			if (this._inScrollDirection === "ltr" || this._inScrollDirection === "rtl") {
				this._snapJumpSpeed = sceneSize.width * SNAP_JUMP_SPEED_FACTOR;
				this._stickyMoveSpeed = sceneSize.width * STICKY_MOVE_SPEED_FACTOR;
			} else {
				this._snapJumpSpeed = sceneSize.height * SNAP_JUMP_SPEED_FACTOR;
				this._stickyMoveSpeed = sceneSize.height * STICKY_MOVE_SPEED_FACTOR;
			}
...
class LayerPile extends Container$1 {
...
class Player {
	...
	constructor(rootElement, backgroundColor = null) {
		...
		this._renderer = new Renderer$1(rootElement, backgroundColor);
...
//# sourceMappingURL=divina.esm.js.map
