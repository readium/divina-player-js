import { Container as Container$1, Graphics, Application, Text, Sprite, Texture as Texture$1, AnimatedSprite, Loader as Loader$1, Rectangle } from 'pixi.js';
...
class Container {
	...
	constructor(name = null, parent = null, pixiContainer = null) {
		this._pixiContainer = pixiContainer || new Container$1();
		this._setName(name);
	...
	addMask() {
		this._maskingPixiContainer = new Container$1();
		this._setMaskingPixiContainerName();
	...
	addChildAtIndex(container, index) {
		...
		const tmpPixiContainer = new Container$1();
		i -= 1;
		while (i >= 0 && zIndex <= i) {
...
class TextureElement extends Container {
	...
	_assignDummyTexture() {
		this._setTexture(Texture$1.WHITE);
	...
	_addPlayableSprite(texturesArray = null) {
		if (texturesArray && texturesArray.length > 0) { // For a sequence slice
			this._playableSprite = new AnimatedSprite(texturesArray);
		} else { // For a video slice
			this._playableSprite = new Sprite();
		}
...
class Texture {
	...
	static createVideoTexture(videoPath) {
		const texture = Texture$1.from(videoPath);
